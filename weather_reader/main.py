...
from download_common import save_to_pq, initial_report, final_report, SERVERS, city_lat_lon
import time
...
def main(city_lat_lon, concur_type=None, max_concur_req=None):
    ...
    if concur_type == "thread":
        result = download_readings_concur(SERVERS["WEATHER"], city_lat_lon, "thread", max_concur_req)
        df = result[0]
        counter = result[1]
    elif concur_type == "process":
        result = download_readings_concur(SERVERS["WEATHER"], city_lat_lon, "process", max_concur_req)
        df = result[0]
        counter = result[1]
    elif concur_type == "coroutine":
        result = download_readings_async(SERVERS["WEATHER"], city_lat_lon, max_concur_req)
        df = result[0]
        counter = result[1]
    else:
